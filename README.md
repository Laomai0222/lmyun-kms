...
* ##### 注：从2020年4月11日起，添加了PGP密钥对数字签名版本，为了安全起见，建议下载PGP加密版本，并用LMYUN_KMS证书进行解密使用或校验LMYUN_KMS.exe（证书大概每周更新一次）
...
> * ###### 1、首先下载证书（以.asc结尾的文件）、Kleopatra客户端和加密后的LMYUN_KMS文件（以.pgp结尾的文件，大致与客户端大小相同）
> * ###### 2、打开Kleopatra，选择导入证书，把LMYUN_KMS证书导入（密码为lmyun_kms）
> * ###### 3、**请认清LMYUN_KMS证书！！！**（名称为LMYUN_KMS，邮箱为KMS@lmyun.top）
> * ###### 4、使用证书进行对软件的解密，即可获取LMYUN_KMS客户端
...
#### 启动后无限循环
> ##### 无法连接KMS服务器，请检测网络连接，可以尝试ping服务器
> `ping kms.lmyun.top`
> ##### 若无法ping通，可以尝试使用网络代理后再ping
#### 启动后闪退
> ##### 1、可能是系统版本不支持，请重新启动或检查系统是否完整，若不为WindowsNT则无法启动
> ##### 2、下载的LMYUN_KMS客户端不完整，请重新下载或使用PGP证书检查是否完整和验证数字签名
#### 无法激活Windows或office
